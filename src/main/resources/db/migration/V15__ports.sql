...
-- ports_seq, so ddl-auto=validate in prod needs it to exist (mirroring V13). The table is empty on
-- first run, so START WITH 1 is safe; INCREMENT BY 50 matches Hibernate's default allocationSize.
...
CREATE SEQUENCE IF NOT EXISTS ports_seq START WITH 1 INCREMENT BY 50 NOCACHE NOCYCLE;
